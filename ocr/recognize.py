...
# 5 - ключ кэша считается по самим буквам, а не по вырезанному куску
#     картинки: сдвиг табло на пиксель больше не обесценивает кэш.
CACHE_VER = 5
...
def _cell_hash(cell, tag):
    """Ключ кэша по самим буквам, а не по вырезанному прямоугольнику.

    Ячейка вырезается по расчётной сетке, и стоит табло сдвинуться на
    пиксель или строке переехать на позицию выше - прямоугольник уже
    другой, хотя написано в нём то же самое. Из-за этого кэш работал
    только пока кадр не менялся: на неизменившемся кадре 88 % попаданий,
    а на новом почти ноль, и прогон вырастал с 15 секунд до 164.

    Поэтому ключ считаем от самого текста: отделяем буквы от фона,
    обрезаем поля и хэшируем получившуюся картинку. Одна и та же фамилия
    даёт один ключ, где бы она на табло ни оказалась.
    """
    ...
    flat = a.reshape(-1, 3)
    colors, counts = np.unique(flat, axis=0, return_counts=True)
    ink = np.abs(a - colors[counts.argmax()]).sum(axis=2) > 90
    # Стираем сплошные линии: в ячейку всегда прихватывается кусок
    # разделителя строк или боковой рамки, и от того, сколько его попало,
    # зависят границы. Без этого ключ снова зависел бы от сдвига.
    if ink.size:
    ...
    h.update(b"%dx%d|" % box.shape)
    h.update(np.packbits(box).tobytes())
    return h.hexdigest()
