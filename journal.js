...
(function (global) {
  ...
  // dz      - аэродром по умолчанию; пока табло одно, это всегда Танай
  ...
  function cfg() {
    ...
    return {
      ...
      ff: c.ff == null ? '' : c.ff,
      ex: c.ex || '',
      // поле заполнено сразу, но остаётся пустым, если его нарочно очистили
      dz: c.dz == null ? 'DZ Tanay' : c.dz,
      base: +c.base || 0,
  ...
  //  canopy:'JFX 84', alt:4000, ff:50, ex:'', task:'ФВ', dz:'DZ Tanay', auto:1}
    ...
    for (k in pend) {
      ...
      if (!hasJump(list, p.date, p.load)) {
        list.push({
          ...
          ex: c.ex || '', task: p.task || '', dz: c.dz || '', auto: 1
        });
        added++; changed = true;
      }
      delete pend[k];
